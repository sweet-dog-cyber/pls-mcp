...
返回: 区域规则列表，含名称、进出基准点数、拟合方式`,
  ...
  try {
    ...
    const fittingMethodMap: Record<number, string> = {
      ...
      content: [{
        ...
        text: JSON.stringify({
          ...
          areaRules: rules.map((r: any) => ({
            ...
            fittingMethodName: fittingMethodMap[r.fitting_method as number] || '',
    ...
    return {
      content: [{ type: 'text', text: `Error: 查询区域规则失败 - ${err.message}` }],
      isError: true,
    };
  }
});
