...
import { QUERY_ANNOTATIONS, FITTING_METHOD_MAP } from '../constants.js';
import { truncateOutput } from '../utils/truncate.js';

server.registerTool('list_area_rules', {
  title: 'list_area_rules',
  description: `【📊 查询】获取区域规则（算法规则）列表，可按名称或拟合方式筛选。
...
提示: 最多返回 2000 条。`,
  ...
  try {
    ...
    if (fittingMethod !== undefined) { sql += ' AND fitting_method = ?'; params.push(fittingMethod); }
    ...
    const { text, truncated } = truncateOutput(JSON.stringify({
      total: rules.length,
      areaRules: rules.map((r: any) => ({
        id: r.id, name: r.name, inside: r.inside, outside: r.outside,
        fittingMethod: r.fitting_method,
        fittingMethodName: FITTING_METHOD_MAP[r.fitting_method as number] || '',
