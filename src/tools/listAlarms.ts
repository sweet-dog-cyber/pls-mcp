...
错误处理:
  ...
  try {
    const ALARM_TYPE_MAP: Record<string, number> = { '入侵': 0, '越界': 1, '超限': 2, '低位': 3, '超时': 4, '低电': 5, '超速': 6 };
    ...
    if (alarmType !== undefined) { sql += ' AND alarm_type = ?'; params.push(ALARM_TYPE_MAP[alarmType] ?? alarmType); }
