...
返回: 带分页的人员列表，含姓名、部门、电话、绑定标签`,
  ...
  try {
    ...
    const params: any[] = [];
    if (departmentId !== undefined) { sql += ' AND department_id = ?'; params.push(departmentId); }
    ...
    if (departmentId !== undefined) { countSql += ' AND department_id = ?'; countParams.push(departmentId); }
    if (keyword !== undefined) { countSql += " AND name LIKE ?"; countParams.push(`%${keyword}%`); }
