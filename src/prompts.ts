...
export function registerPrompts(server: McpServer) {
  ...
  server.registerPrompt(
    ...
    async (args: { tagCode?: string; alarmId?: number }) => {
      ...
      try {
        if (args.alarmId) {
          const alarms = await query(
            `SELECT tag_code, alarm_type, area_id, area_name, alarm_time, content
             FROM log_alarm WHERE id = ? LIMIT 1`,
            [args.alarmId]
          );
          if (alarms.length) {
        ...
        if (args.tagCode) {
          const recentAlarms = await query(
            `SELECT tag_code, alarm_type, area_name, alarm_time
             FROM log_alarm WHERE tag_code = ? ORDER BY alarm_time DESC LIMIT 5`,
            [args.tagCode]
          );
      ...
      return {
        messages: [{ role: 'user' as const, content: { type: 'text' as const, text: contextParts.join('\n') } }],
      };
    }
  );
  ...
  server.registerPrompt(
    ...
    async (args: { departmentId?: number; date?: string }) => {
      const contextParts: string[] = [
        '# 人员在场报告\n',
      ...
      try {
        ...
        const personnel = await query(
          `SELECT id, name, department_id, tag_code
           FROM personnel_information WHERE 1=1${deptCondition}`,
          deptParams
        );
        ...
        const inArea = await query(
          `SELECT l.tag_code, l.area_id, l.area_name, l.in_date_time
           FROM logs_area_in_and_out l WHERE DATE(l.in_date_time) = ? AND l.out_date_time IS NULL`,
          [dateFilter]
        );
    ...
    async (args: { areaId?: number }) => {
      ...
      try {
        ...
        const areas = await query(
          `SELECT id, name, map_id, area_type FROM gis_area${areaCondition}`,
          areaParams
        );
        contextParts.push(`## 区域列表（共 ${areas.length} 个区域）`);
        contextParts.push(JSON.stringify(areas, null, 2));

        const alarmStats = await query(
          `SELECT area_id, area_name, COUNT(*) as alarm_count
        ...
        const inOutStats = await query(
          `SELECT area_id, area_name, COUNT(*) as person_count
