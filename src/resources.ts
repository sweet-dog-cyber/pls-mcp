...
export function registerResources(server: McpServer) {
  ...
  server.registerResource(
    'pls_realtime_map',
    new ResourceTemplate('pls://realtime-map/{mapCode}', { list: undefined }),
    {
    ...
    async (uri: URL, variables: Record<string, string | string[]>) => {
      try {
        ...
        const positions = allLocations.filter((p: any) => p.mapCode === mapCode);

        let bindingsMap: Record<string, string> = {};
        try {
          const bindData = await callTagBindings();
          if (typeof bindData === 'object' && !Array.isArray(bindData)) {
            bindingsMap = bindData as Record<string, string>;
          } else if (Array.isArray(bindData)) {
            bindData.forEach((b: any) => { bindingsMap[b.tagCode] = b.bindName; });
          }
        } catch {}
    ...
    async (uri: URL) => {
      try {
        const rows = await query(
          `SELECT id, name, alarm_type, thresholds, monitored_period
        ...
        return {
          contents: [{
            ...
            text: JSON.stringify(rows.map((r: any) => ({
              ...
              alarmTypeName: alarmTypeMap[r.alarm_type as number] || '',
